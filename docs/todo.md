...
| L5  | ⬜     | Plugin host: interface/ID DI framework (plugins.md) — export & discover    |
...
- **L5** ⬜ — **Plugin host — interface/ID DI framework** ([`docs/plugins.md`](plugins.md)): a set of versioned interfaces (`IFrontend`, `IRng`, `IStorageEngine`, …); a plugin **exports** any subset (COM-lite `query` by ID); the host **discovers** exports and wires them, and is a **service container** (`resolve(id)` for cross-plugin DI). C++ interfaces + `extern "C"` ABI-versioned factory; loader = built-in providers + `dlopen`; front-ends contribute CLI args (`options()` → CLI11); CLI11+spdlog host layer; `null` test plugin (exports `IFrontend` + a test `IRng`). L2 NFS front-ends and the LMDB/mem engines and rng generators all become exported interfaces.
