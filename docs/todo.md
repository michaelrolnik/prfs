...
- **L2** ✅ — **NFS front-end** shipped as the `nfsv3` plugin (`plugins/nfsv3`): the full v3 procedure set + MOUNT program on standalone-Asio coroutines with an own io-thread pool (the self-contained XDR + thread-pool TCP-server option from the analysis; not `rpcgen`, not Ganesha). 16-byte `(nodeID, snapId)` filehandles via `nodeById`; snapshot views read-only (`ROFS`); `..` via-parent; AUTH_SYS creds stamped; permissive ACCESS; synthesized `.snapshot` LOOKUP-able but hidden from readdir. `tests/nfsv3/`. READDIR/READDIRPLUS page via the store's stable `readdirPage` name-cursor (O(page), immune to mid-scan add/remove); NFS's 64-bit cookie ↔ name bridge is a per-name hash + a bounded `cookie→name` cache (miss ⇒ scan, else `BAD_COOKIE`). Fixes **B11**. *Remaining carve-outs:* single `"/"` MOUNT export; no rpcbind/NLM (mount with explicit ports + `nolock`); directory-`nlink` mode still POSIX-compat only. **nfsv4** (`plugins/nfsv4`): an NFSv4.0 COMPOUND server, first increment = the browse/read path (PUTROOTFH/PUTFH/GETFH/SAVEFH/RESTOREFH, LOOKUP/LOOKUPP, ACCESS, GETATTR with fattr4 attribute bitmaps, READLINK, READ, READDIR, and minimal SETCLIENTID/RENEW/OPEN(no-create)/CLOSE state); same 16-byte fh; no MOUNT program; unimplemented ops → `NFS4ERR_NOTSUPP`. RPC-level test `tests/nfsv4/` (COMPOUND PUTROOTFH→LOOKUP→GETATTR→READ). *Follow-ups:* the write surface, real OPEN/lock state, the full attribute set + a `readdirPage` name-cursor for kernel-mount conformance. **Conformance:** `scripts/pjdfstest.sh` runs the pjdfstest POSIX suite over a mount (`sudo scripts/pjdfstest.sh [category]`) — structural categories largely pass; permission/ownership/timestamp-vs-wallclock failures are by design (no mode enforcement, deterministic clock). It surfaced and fixed **B10** (parent-dir mtime/ctime on namespace change), **B11** (READDIR skip/dup under mid-scan mutation → stable name-cursor pager), and **B12** (SETATTR dropped sub-second mtime/atime → `atimeNs`/`mtimeNs` now round-trip). Remaining failures are by-design (no mode enforcement) or NFSv3-inherent (uint32 `nfstime3` seconds, silly-rename on open-unlink).
- **L3** ⬜ — **GC / compaction**: reclaim superseded link-set versions + unreferenced `content`/`strings`.
- **L4** 🚧 — **Footprint stats.** *Cheap half done:* the sparse/allocated footprint is the sum of `content::allocatedBlocks` (st_blocks, holes excluded) over the distinct REG nodes — a physical, hole-aware "what a backup target stores" number. Surfaced by the `bigtree` plugin's build summary (`content footprint — X allocated of Y logical (Z% saved by sparse holes); dedup D% (modeled)`); aggregate ratio tested in `tests/content` (`AllocatedAggregateTracksSparseRatio`). *Deferred:* true cross-file dedup counting needs distinct-block refcounting (better computed analytically from `dedupPercent`+corpus than counted); the configured dedup% is surfaced as a hint for now. *Also done:* per-file `used`/st_blocks over the mount is sparse-aware — nfsv3 links content and publishes the store's policy once at `start()`, so fattr3 `used` = `allocatedBlocks*512` (holes excluded) and `du` reflects the real footprint for any store (not just bigtree's). No ABI bump: the policy is a process-singleton read lock-free by the fattr encoder (`tests/nfsv3 UsedReflectsSparseAllocation`).
