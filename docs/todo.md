...
- **L5a** ⬜ — **DI registry** ([`docs/di.md`](di.md)): a typed C++ container keyed by `(interface-id, flavor)` — `provide`/`resolve`/`resolveAll`/`requireAllResolved`; fail-loud on unresolved; a `Registry` *is* a scope (RAII, for hermetic tests). Concepts borrowed from the vendor `ref_di` reference (named binding, flavors, sentinel, scopes) but typed/RAII/explicit — no `void*` vtables, no constructor macros, no dlopen/teardown machinery. `include/prfs/di.hpp`. The `rng` registry is its working prototype.
- **L5b** ⬜ — **Plugin host** ([`docs/plugins.md`](plugins.md)) over the DI registry: versioned interfaces (`IFrontend`/`IRng`/`IStorageEngine`); a plugin **provides** its implementations into the registry (flavor = variant), the host **resolves** (`resolveAll<IFrontend>` → start; engine/rng by flavor). C++ interfaces + `extern "C"` ABI-versioned factory; loader = built-in providers + `dlopen` (+ `withdraw` on unload); front-ends add CLI args (`options()` → CLI11); CLI11+spdlog host layer; `null` test plugin. L2 NFS front-ends and the LMDB/mem engines and rng generators are all providers.
